...
namespace internals {
    ...
    bool WebsocketsEndpoint::send(const char* data, const size_t len, const uint8_t opcode, const bool fin, const bool mask, const char* maskingKey) {
		...
		else // Can be sent in two parts and save unnecessary memory allocation
		{
			this->_client->send(reinterpret_cast<const uint8_t*>(message_data.c_str()), message_data.size()); // send the header
			this->_client->send(reinterpret_cast<const uint8_t*>(data), len); // Send the body
		}
